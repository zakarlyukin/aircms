...
void display_debug(const String& text) {
	...
	if (has_display) {
		...
		display.clear();
		display.displayOn();
		display.setFont((const uint8_t*)Monospaced_plain_9);
...
#define strcpyFromJSON(key) if (json.containsKey(#key)) strcpy(key, json[#key]);
          ...
          strcpyFromJSON(capmsid);
          strcpyFromJSON(capmspwd);
...
#define copyToJSON_String(varname) json_string +="\""+String(#varname)+"\":\""+String(varname)+"\",";
  ...
  copyToJSON_String(capmsid);
  copyToJSON_String(capmspwd);
...
void webserver_config() {
    ...
    /* Config for CAPMS (Пока не надо)*/
    page_content += F("<!--<table>");
    page_content += form_input(F("capmsid"), F("CAPMS ID"), capmsid, 64);
    page_content += form_password(F("capmspwd"), FPSTR(INTL_PASSWORT), capmspwd, 64);
    page_content += form_submit(FPSTR(INTL_SPEICHERN));
    page_content += F("</table><br/><hr/><b>"); //--> Config for CAPMS

    debug_out(F("  end output config page 1"), DEBUG_MIN_INFO, 1);
...
//		page_content += form_checkbox("auto_update", FPSTR(INTL_AUTO_UPDATE), auto_update);
		...
		page_content += form_submit(FPSTR(INTL_SPEICHERN));
		page_content += F("</table><br/>");
		page_content += F("<br/>--></form>"); //Пока ВСЁ скрыть
		if (WiFi.status() != WL_CONNECTED) {  // scan for wlan ssids
...
#define readPasswdParam(param) if (server.hasArg(#param)){ i = 0; masked_pwd = ""; for (i=0;i<server.arg(#param).length();i++) masked_pwd += "*"; if (masked_pwd != server.arg(#param) || server.arg(#param) == "") { server.arg(#param).toCharArray(param, sizeof(param)); } }
    ...
    readCharParam(capmsid);
    readPasswdParam(capmspwd);
...
#undef readIntParam
  ...
  wdt_enable(30000);// 30 sec
	  server.send(200, FPSTR(TXT_CONTENT_TYPE_TEXT_HTML), page_content);
    debug_out(F("  send output config page 1"), DEBUG_MIN_INFO, 1);
  wdt_reset();
}
...
void webserver_images() {
	if (server.hasArg("name")) {
		if (server.arg("name") == "luftdaten_logo") {
			...
			server.send(200, FPSTR(TXT_CONTENT_TYPE_IMAGE_SVG), FPSTR(LUFTDATEN_INFO_LOGO_SVG_OUR));
...
void loop() {
  ...
  String result_iaqcore = "";
    ...
    debug_out(F("## Sending to us: "), DEBUG_MIN_INFO, 1);
...
//          token = String(capmspwd);
  ...
  sendData(our_data, pin, host_our, httpPort_our, char_full_url, "", contentType);
}
